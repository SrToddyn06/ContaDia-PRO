...
const NeonCard = ({ children, className = "", glowColor = "green" }: { children: React.ReactNode, className?: string, glowColor?: 'green' | 'blue' | 'pink' }) => {
  const glowClasses = {
...
const NeonButton = ({ children, onClick, variant = 'green', className = "", disabled = false }: any) => {
  const variants = {
    ...
    ghost: 'bg-transparent text-white/40 hover:text-white hover:bg-white/5 border-transparent'
  };
...
export default function App() {
  ...
  const fetchSettings = useCallback(async () => {
    try {
      ...
      if (data) setSettings(s => ({ ...s, ...data, 
        half_day_value: Number(data.half_day_value), full_day_value: Number(data.full_day_value),
        show_jokes: data.show_jokes === 'true', show_tips: data.show_tips === 'true',
        weekly_goal: Number(data.weekly_goal), monthly_goal: Number(data.monthly_goal)
      }));
    } catch (e) {}
  }, []);
  ...
  const addLog = (type: 'half_day' | 'full_day') => {
    const value = type === 'half_day' ? settings.half_day_value : settings.full_day_value;
    apiCall('/api/logs', { date: new Date().toISOString(), type, value });
  ...
  const importCSV = async (event: React.ChangeEvent<HTMLInputElement>) => {
    ...
    reader.onload = async (e) => {
      ...
      for (let i = startIdx; i < lines.length; i++) {
        const [dateStr, typeStr, valueStr] = lines[i].split(',');
        if (!dateStr || !typeStr || !valueStr) continue;
        let date;
        if (dateStr.includes('/')) {
        ...
        if (!isNaN(value)) newLogs.push({ date, type, value });
      }
      if (newLogs.length > 0) {
        for (const log of newLogs) await fetch('/api/logs', { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify(log) });
        fetchLogs();
  ...
  const stats = useMemo(() => {
    ...
    const now = new Date();
    const getEarned = (start: Date) => active.filter(l => parseISO(l.date) >= start).reduce((a, c) => a + c.value, 0);
    ...
    return {
      total: active.reduce((a, c) => a + c.value, 0),
      days: active.length,
      weekly: wEarned,
      monthly: mEarned,
  ...
  const exportCSV = (range: 'all' | 'month' | 'day' = 'all') => {
    ...
    const headers = ['Data', 'Hora', 'Dia da Semana', 'Tipo', 'Valor (R$)', 'Mês', 'Ano'];
  ...
  const importAdminCSV = async (event: React.ChangeEvent<HTMLInputElement>) => {
    ...
    reader.onload = (e) => {
      ...
      for (let i = startIdx; i < lines.length; i++) {
        ...
        const value = parseFloat(valueStr) || (type === 'full_day' ? settings.full_day_value : settings.half_day_value);
      ...
      alert(`${Object.keys(empMap).length} funcionários processados!`);
    };
    reader.readAsText(file);
    event.target.value = '';
  };
  ...
  return (
    <div className="min-h-screen bg-app-bg text-white font-sans">
      <div className="max-w-md mx-auto pb-32 pt-12 px-6">
        <header className="flex items-center justify-between mb-12">
          <div>
            ...
            <div className="flex items-center gap-2 mt-1">
              ...
              <span className="text-[10px] font-mono text-white/40 uppercase tracking-[0.3em]">Sistema Ativo</span>
            </div>
          </div>
        ...
        <AnimatePresence mode="wait">
            ...
            <motion.div key="db" initial={{ opacity: 0, scale: 0.98, filter: 'blur(10px)' }} animate={{ opacity: 1, scale: 1, filter: 'blur(0px)' }} exit={{ opacity: 0, scale: 1.02, filter: 'blur(10px)' }} className="space-y-8">
              <div className="grid grid-cols-1 gap-4">
                <NeonCard glowColor="green" className="relative overflow-hidden group">
                  ...
                  <p className="text-[10px] font-bold text-white/40 uppercase tracking-widest mb-1">Balanço Total</p>
                  <h2 className="text-5xl font-black tracking-tighter">R$ <NumberTicker value={stats.total} /></h2>
                </NeonCard>
                <div className="grid grid-cols-2 gap-4">
                  <NeonCard glowColor="blue"><p className="text-[10px] font-bold text-white/40 uppercase tracking-widest mb-1">Sessões</p><p className="text-3xl font-black">{stats.days}</p></NeonCard>
                  <NeonCard glowColor="pink"><p className="text-[10px] font-bold text-white/40 uppercase tracking-widest mb-1">Meta Sem.</p><p className="text-3xl font-black">{stats.wProgress.toFixed(0)}%</p></NeonCard>
                </div>
              </div>
              ...
              <div className="space-y-6">
                {[ { label: 'Semanal', val: stats.weekly, goal: settings.weekly_goal, prog: stats.wProgress, color: 'green' },
                  ...
                  <div key={i} className="space-y-3">
                    <div className="flex justify-between items-end"><span className="text-[10px] font-bold text-white/40 uppercase">Progresso {p.label}</span><span className={`text-xs font-mono text-neon-${p.color}`}>R$ {p.val} / {p.goal}</span></div>
                    <div className="w-full h-2 bg-white/5 rounded-full overflow-hidden border border-white/5">
                      <motion.div initial={{ width: 0 }} animate={{ width: `${p.prog}%` }} transition={{ type: 'spring', stiffness: 50, damping: 20 }} className={`h-full bg-gradient-to-r from-neon-${p.color} to-neon-blue neon-glow-${p.color} relative`}>
                        <motion.div animate={{ x: ['-100%', '200%'], opacity: [0, 0.3, 0] }} transition={{ repeat: Infinity, duration: 2 }} className="absolute inset-0 w-20 bg-white/30 skew-x-12" />
                      </motion.div>
                    </div>
                  </div>
                ))}
              ...
              <p className="text-center py-6 px-4 text-sm text-white/40 italic">"{phrase}"</p>
          ...
          {activeTab === 'calendar' && (
            <motion.div key="cal" initial={{ opacity: 0, scale: 0.98, filter: 'blur(10px)' }} animate={{ opacity: 1, scale: 1, filter: 'blur(0px)' }} exit={{ opacity: 0, scale: 1.02, filter: 'blur(10px)' }} className="space-y-6">
              <h2 className="text-[10px] font-bold text-white/40 uppercase tracking-[0.3em]">Calendário de Atividades</h2>
              <div className="grid grid-cols-3 gap-3">
                {[ { l: 'Semanal', v: stats.weekly, c: 'green' }, { l: 'Mensal', v: stats.monthly, c: 'blue' }, { l: 'Anual', v: stats.yearly, c: 'pink' } ].map((s, i) => (
                  <div key={i} className="p-4 rounded-2xl bg-white/5 border border-white/5"><p className="text-[8px] font-bold text-white/30 uppercase mb-1">{s.l}</p><p className={`text-sm font-bold text-neon-${s.c}`}>R$ {s.v}</p></div>
                ))}
              </div>

              <div className="bg-app-card border border-white/5 rounded-[2rem] p-6">
                <div className="flex items-center justify-between mb-6">
                  <h3 className="text-sm font-bold uppercase text-white/60">{format(currentMonth, 'MMMM yyyy', { locale: ptBR })}</h3>
                  <div className="flex gap-2">
                    <button onClick={() => setCurrentMonth(subMonths(currentMonth, 1))} className="p-2 rounded-xl bg-white/5"><ChevronLeft className="w-4 h-4" /></button>
                    <button onClick={() => setCurrentMonth(addMonths(currentMonth, 1))} className="p-2 rounded-xl bg-white/5"><ChevronRight className="w-4 h-4" /></button>
                  </div>
                </div>
                <div className="grid grid-cols-7 gap-1 mb-2">{['D','S','T','Q','Q','S','S'].map((d, i) => <div key={i} className="text-center text-[10px] font-bold text-white/20">{d}</div>)}</div>
                    ...
                    return days.map((d, i) => {
                      ...
                      return (
                        <motion.button key={i} whileTap={{ scale: 0.9 }} onClick={() => setSelectedDate(d)}
                          className={`relative h-10 flex flex-col items-center justify-center rounded-xl text-xs font-bold ${!isSameMonth(d, currentMonth) ? 'text-white/10' : 'text-white/60'} ${sel ? 'bg-neon-green/20 text-neon-green border border-neon-green/30' : 'hover:bg-white/5'} ${dLogs.length > 0 && !sel ? 'text-neon-green' : ''}`}>
                          {format(d, 'd')}
              ...
              <div className="space-y-4">
                <h3 className="text-[10px] font-bold text-white/40 uppercase">{selectedDate ? format(selectedDate, "dd 'de' MMMM", { locale: ptBR }) : 'Selecione uma data'}</h3>
                <div className="space-y-3">
                  {(() => {
                    const dLogs = selectedDate ? logs.filter(l => isSameDay(parseISO(l.date), selectedDate)) : [];
                    return dLogs.length ? dLogs.map((l, i) => (
                      <motion.div key={l.id} initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: i * 0.05 }} className="flex items-center justify-between p-5 rounded-2xl bg-white/5 border border-white/5">
                        <div className="flex items-center gap-4">
                          ...
                          <div><div className="flex items-center gap-2"><p className="text-sm font-bold">{l.type === 'full_day' ? 'Dia Inteiro' : 'Meio Dia'}</p><div className="flex items-center gap-1 px-2 py-0.5 rounded-full bg-white/5"><Clock className="w-2.5 h-2.5 text-white/40" /><span className="text-[9px] font-mono text-white/60">{format(parseISO(l.date), 'HH:mm')}</span></div></div><p className="text-[10px] text-white/30 uppercase">Verificado</p></div>
                        </div>
                    ...
                    )) : <div className="py-12 text-center border border-dashed border-white/10 rounded-3xl text-white/20 italic text-sm">Nenhum pagamento nesta data.</div>;
                  })()}
                </div>
              </div>
            </motion.div>
          ...
          {activeTab === 'settings' && (
            <motion.div key="set" initial={{ opacity: 0, scale: 0.98, filter: 'blur(10px)' }} animate={{ opacity: 1, scale: 1, filter: 'blur(0px)' }} exit={{ opacity: 0, scale: 1.02, filter: 'blur(10px)' }} className="space-y-8">
              <h2 className="text-[10px] font-bold text-white/40 uppercase tracking-[0.3em]">Ajustes do Sistema</h2>
              <div className="space-y-6">
                <div className="space-y-4"><p className="text-[10px] font-bold text-white/40 uppercase">Valores de Turno</p>
                  <div className="grid grid-cols-2 gap-4">
                    {[ { l: 'Meio Dia', k: 'half_day_value', c: 'blue' }, { l: 'Dia Inteiro', k: 'full_day_value', c: 'green' } ].map((f, i) => (
                      <div key={i} className="space-y-2"><span className="text-[10px] text-white/30 uppercase">{f.l}</span><motion.input whileFocus={{ scale: 1.02 }} type="number" value={settings[f.k as keyof AppSettings] as any} onChange={(e) => apiCall('/api/settings', { [f.k]: Number(e.target.value) }).then(fetchSettings)} className={`w-full p-4 rounded-2xl bg-white/5 border border-white/10 focus:border-neon-${f.c} outline-none font-mono font-bold text-neon-${f.c}`} /></div>
                    ))}
                  </div>
                </div>
                {[ { l: 'Meta Semanal', k: 'weekly_goal', c: 'pink' }, { l: 'Meta Mensal', k: 'monthly_goal', c: 'blue' } ].map((f, i) => (
                  <div key={i} className="space-y-2"><p className="text-[10px] font-bold text-white/40 uppercase">{f.l} (R$)</p><motion.input whileFocus={{ scale: 1.01 }} type="number" value={settings[f.k as keyof AppSettings] as any} onChange={(e) => apiCall('/api/settings', { [f.k]: Number(e.target.value) }).then(fetchSettings)} className={`w-full p-4 rounded-2xl bg-white/5 border border-white/10 focus:border-neon-${f.c} outline-none font-mono font-bold text-neon-${f.c} text-2xl`} /></div>
                ))}
                <div className="space-y-4 pt-4 border-t border-white/5">
                  {[ { l: 'Ativar Piadas', k: 'show_jokes', i: Coffee, c: 'blue' }, { l: 'Dicas de Produtividade', k: 'show_tips', i: TrendingUp, c: 'green' } ].map((t, i) => (
                    <label key={i} className="flex items-center justify-between p-4 rounded-2xl bg-white/5 border border-transparent hover:border-white/10 cursor-pointer group">
                      <div className="flex items-center gap-3"><t.i className={`w-4 h-4 text-white/30 group-hover:text-neon-${t.c}`} /><span className="text-sm font-medium">{t.l}</span></div>
                      <input type="checkbox" checked={settings[t.k as keyof AppSettings] as any} onChange={(e) => apiCall('/api/settings', { [t.k]: e.target.checked }).then(fetchSettings)} className={`w-10 h-5 rounded-full bg-white/10 appearance-none checked:bg-neon-${t.c} transition-all relative before:content-[''] before:absolute before:w-3 before:h-3 before:bg-white before:rounded-full before:top-1 before:left-1 checked:before:left-6`} />
                    </label>
                  ))}
                </div>
              </div>
              <section className="pt-12 border-t border-white/5 space-y-6">
                <div className="space-y-4">
                  <p className="text-[10px] font-bold text-white/40 uppercase tracking-widest">Exportar Dados (CSV)</p>
                  <div className="grid grid-cols-3 gap-2">
                    <button onClick={() => exportCSV('day')} className="flex flex-col items-center gap-2 p-3 rounded-2xl bg-white/5 border border-white/5 hover:border-neon-green/30 transition-all group">
                      <Clock className="w-4 h-4 text-white/30 group-hover:text-neon-green" />
                      <span className="text-[8px] font-bold uppercase">Dia Sel.</span>
                    </button>
                    <button onClick={() => exportCSV('month')} className="flex flex-col items-center gap-2 p-3 rounded-2xl bg-white/5 border border-white/5 hover:border-neon-blue/30 transition-all group">
                      <CalendarIcon className="w-4 h-4 text-white/30 group-hover:text-neon-blue" />
                      <span className="text-[8px] font-bold uppercase">Mês Atual</span>
                    </button>
                    <button onClick={() => exportCSV('all')} className="flex flex-col items-center gap-2 p-3 rounded-2xl bg-white/5 border border-white/5 hover:border-neon-pink/30 transition-all group">
                      <Download className="w-4 h-4 text-white/30 group-hover:text-neon-pink" />
              ...
              {!isAdminAuthenticated ? (
                <div className="space-y-6 py-12">
                  <div className="text-center space-y-2">
                    ...
                    <p className="text-white/40 text-sm">Digite a senha mestra para acessar o painel administrativo.</p>
                  </div>
                  <div className="space-y-4">
                    <input 
                      type="password" 
                      ...
                      className="w-full p-4 rounded-2xl bg-white/5 border border-white/10 focus:border-neon-pink outline-none text-center font-bold tracking-widest"
                    />
                    <NeonButton onClick={() => adminPassword === 'admin123' ? setIsAdminAuthenticated(true) : alert('Senha Incorreta')} variant="pink" className="w-full">
                      Autenticar
                    </NeonButton>
                  </div>
                </div>
                ...
                <div className="space-y-8">
                  <div className="flex items-center justify-between">
                    <h2 className="text-[10px] font-bold text-white/40 uppercase tracking-[0.3em]">Painel do Empregador</h2>
                  ...
                  <NeonCard glowColor="pink" className="relative overflow-hidden">
                    <p className="text-[10px] font-bold text-white/40 uppercase tracking-widest mb-1">Total Geral a Pagar</p>
                  ...
                  <div className="space-y-4">
                    <div className="flex items-center justify-between">
                      <p className="text-[10px] font-bold text-white/40 uppercase tracking-widest">Folha de Pagamento</p>
                      <div className="relative">
                    ...
                    <div className="space-y-3">
                      {employees.length > 0 ? employees.map((emp) => (
                        <motion.div key={emp.id} initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} className="p-5 rounded-2xl bg-white/5 border border-white/5 space-y-4">
                          <div className="flex items-center justify-between">
                            <div className="flex items-center gap-3">
                              ...
                              <div>
                                ...
                                <p className="text-[10px] text-white/30 uppercase">{emp.logs.length} Turnos Registrados</p>
                              </div>
                            </div>
                            ...
                            {emp.logs.slice(0, 3).map((l, idx) => (
                              <div key={idx} className="flex items-center justify-between text-[10px] text-white/40">
                                <div className="flex items-center gap-2">
                                  ...
                                  <span className="px-1.5 py-0.5 rounded bg-white/5 border border-white/5">{l.type === 'full_day' ? 'Integral' : 'Meio'}</span>
                                </div>
                                <span className="font-mono">R$ {l.value}</span>
                              </div>
                            ))}
                            {emp.logs.length > 3 && <p className="text-[8px] text-center text-white/20 uppercase tracking-widest pt-1">+ {emp.logs.length - 3} outros registros</p>}
                          </div>
                        </motion.div>
                      )) : (
                        <div className="py-12 text-center border border-dashed border-white/10 rounded-3xl">
                          ...
                          <p className="text-white/20 italic text-sm">Nenhum funcionário importado.</p>
                          <p className="text-[8px] text-white/10 uppercase mt-2">Importe um CSV com: Nome, Data, Tipo, Valor</p>
                        </div>
                      )}
                    </div>
                  </div>
        ...
        <nav className="fixed bottom-8 left-1/2 -translate-x-1/2 w-[calc(100%-3rem)] max-w-sm glass-nav rounded-3xl p-2 flex items-center justify-between shadow-2xl z-50 border border-white/10">
          {[ 
            ...
            <button key={t.id} onClick={() => setActiveTab(t.id as any)} className="relative flex-1 flex flex-col items-center gap-1 py-3 rounded-2xl z-10">
              ...
              <t.i className={`w-5 h-5 ${activeTab === t.id ? `text-${t.c}` : 'text-white/30'}`} />
              <span className={`text-[8px] font-black uppercase tracking-widest ${activeTab === t.id ? `text-${t.c}` : 'text-white/30'}`}>{t.l}</span>
            </button>
          ))}
        </nav>
          ...
          {modals.reset && (
            <div className="fixed inset-0 z-[100] flex items-center justify-center p-6 bg-black/90 backdrop-blur-sm">
              <motion.div initial={{ scale: 0.9, opacity: 0 }} animate={{ scale: 1, opacity: 1 }} exit={{ scale: 0.9, opacity: 0 }} className="bg-app-card border border-neon-pink/30 rounded-[2.5rem] p-10 max-w-sm w-full text-center neon-glow-pink">
                ...
                <p className="text-white/40 text-sm mb-8">Isso zerará o seu saldo e progressos no painel inicial. O histórico no calendário será mantido.</p>
                <div className="flex flex-col gap-3">
                  <button onClick={async () => { await apiCall('/api/reset'); setModals(m => ({ ...m, reset: false })); fetchSettings(); }} className="w-full py-4 rounded-2xl bg-neon-pink text-white font-bold">Confirmar Reset</button>
                  <button onClick={() => setModals(m => ({ ...m, reset: false }))} className="w-full py-4 rounded-2xl bg-white/5 text-white/60 font-bold">Cancelar</button>
                </div>
              </motion.div>
            </div>
          )}
          {modals.factory && (
            <div className="fixed inset-0 z-[100] flex items-center justify-center p-6 bg-black/90 backdrop-blur-sm">
              <motion.div initial={{ scale: 0.9, opacity: 0 }} animate={{ scale: 1, opacity: 1 }} exit={{ scale: 0.9, opacity: 0 }} className="bg-app-card border border-rose-500/30 rounded-[2.5rem] p-10 max-w-sm w-full text-center neon-glow-pink">
                ...
                <p className="text-white/40 text-sm mb-8">CUIDADO: Isso apagará TUDO (ganhos e configurações) e voltará aos padrões originais.</p>
                <div className="flex flex-col gap-3">
                  <button onClick={async () => { await apiCall('/api/factory-reset'); window.location.reload(); }} className="w-full py-4 rounded-2xl bg-rose-500 text-white font-bold shadow-[0_0_20px_rgba(244,63,94,0.4)]">WIPE TOTAL</button>
                  <button onClick={() => setModals(m => ({ ...m, factory: false }))} className="w-full py-4 rounded-2xl bg-white/5 text-white/60 font-bold">Cancelar</button>
                </div>
              </motion.div>
            </div>
          )}
